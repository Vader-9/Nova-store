...
function Electronics({ products, likes, favourites, setFavourites, searchItem, setSearchItem, cartItems, setCartItems, setDescriptionItem }) {
...
if (electronicsItems.length === 0) {
  ...
  const fitteredSearchItems = electronicsItems.filter((item) => item.title.toLowerCase().includes(search.toLocaleLowerCase()))



  const electro = searchItem ? fitteredSearchItems : electronicsItems;
      ...
      {!likes ? (
        ...
        <div>
          ...
          <div className="w-full h-[80px] p-4 my-4 flex justify-center items-center gap-2">
            <h1 class="text-4xl md:text-6xl font-extrabold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 text-transparent bg-clip-text drop-shadow-lg">Electronics</h1>
